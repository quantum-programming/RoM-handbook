...
\begin{tikzpicture}[xscale=0.75]
    \draw[rectangle, fill=white, draw=black, line width=3pt]
    (-4, 7.25) rectangle (66, -19);

    \node[scale=5] at (31, 5) {
        $\mathcal{R}(\rho) \defeq \min_{{\color{cD}\sigma_+}, {\color{cA}\sigma_-} \in\mathrm{STAB}_n}\left\{2p+1 \relmiddle| \rho = (p+1) {\color{cD}\sigma_+} - p {\color{cA}\sigma_-} ,~ p\geq 0\right\}$
    };
    ...
    % \sigma_+
    % 2,  6, 18, 20, 23, 26, 30, 38, 44, 48, 55
    % \sigma_-
    % 9, 22, 27, 43, 51

    \foreach \col in {2, 6, 18, 20, 23, 26, 30, 38, 44, 48, 55}{
    \draw[cD,-{Latex[length=5mm, width=8mm]},line width=8pt] (\col + 0.5, 1.8) -- (\col + 0.5, 0.3);
    }
    \foreach \col in {9, 22, 27, 43, 51}{
    \draw[cA,-{Latex[length=5mm, width=8mm]},line width=8pt] (\col + 0.5, 1.8) -- (\col + 0.5, 0.3);
    }

    \node[scale=4] at (57.5, 3) {Sparse Solution};
